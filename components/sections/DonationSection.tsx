              ...
              <div className="relative aspect-square lg:aspect-auto min-h-[300px] lg:min-h-[400px]">
                ...
                <div className="absolute bottom-6 left-6 right-6 z-10">
                  <h3 className="text-2xl font-bold text-white mb-1">
                    {currentImpact.label === "Scholarship"
                      ? "Full Scholarship"
                      : currentImpact.label}
                  </h3>
                  <p className="text-gray-200">{currentImpact.description}</p>
                </div>
              </div>
                  ...
                  <div className="relative mb-4">
                    ...
                    <div className="absolute top-1/2 -translate-y-1/2 w-full flex justify-between">
                      {[10, 30, 50, 70, 90].map((position, index) => (
                        <div
                          key={index}
                          className="absolute h-3 w-3 bg-gray-300 rounded-full transition-all duration-300"
                          style={{
                            left: `${position}%`,
                    ...
                    <div
                      className="absolute top-1/2 -translate-y-1/2 w-5 h-5 bg-primary rounded-full shadow-lg transition-all duration-300 ease-out z-20 pointer-events-none"
                      style={{
                        left: `${[10, 30, 50, 70, 90][selectedLevel]}%`,
                        transform: "translate(-50%, -50%)",
                      }}
                    />
                  </div>
                  ...
                  <div className="flex justify-between text-sm relative">
                    {impactLevels.map((level, index) => (
                      <button
                        ...
                        className={`flex-1 text-center transition-all duration-300 cursor-pointer hover:text-primary ${
                          selectedLevel === index
